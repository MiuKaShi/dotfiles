...
int magicOpen(std::string filename);
int magicExec(std::string filename);
int magicCopy(std::string filename);
int magicMove(std::string filename);
int magicRemove(std::string filename);
void help();
void version();
...
int magicOpen(std::string filename) {
  ...
  if (ext == "epub" && mineType != "application/epub+zip") {
