...
function toggleRoundCorners(model, num) roundCorners = not roundCorners end
  ...
  for i, obj, sel, layer in p:objects() do
    if sel and obj:type() == "path" then
      for _, subPath in ipairs(obj:shape()) do
        ...
        if not area then path = ipe.Path(model.attributes, { curve }) end
...
function offsetCurve(segs, dist, closed)
  ...
  if closed then segs[#segs + 1] = { segs[#segs][2], segs[1][1] } end
  ...
  for i, seg in ipairs(newSegs) do
    ...
    if not (closed and i == 1) then curve[#curve + 1] = { type = "segment", seg[1], seg[2] } end

    -- create the connecting arc
    if arc then curve[#curve + 1] = arc end
  end
...
function nextSegment(segments, i, closed)
  ...
  if not next and closed then next = segments[1] end
  return next
end
