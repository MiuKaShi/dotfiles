...
if _G.config.version < "7.1.7" then beziername = "bezier" end
...
function _G.ipeplot_tick_range(f, t, s)
  local a = {}
  if (s == 0) or (f * s >= t * s) then return a end
  local x = f
  local i = 1
  while x <= t do
    a[i] = x
...
local function isfinite(x) return (_G.type(x) == "number") and (x > -math.huge) and (x < math.huge) end
...
local function get_dialog_parent(model)
  local ui = model.ui
  if ui.win == nil then return ui end
  return ui:win()
end
...
local function cubicfit(t0, t1, n, y)
  local d, p0, p1, p2, p3 = {}, {}, {}, {}, {}
  local h = (t1 - t0) / (n - 1)
  if n == 2 then return y[1], 2.0 * y[1] / 3 + y[2] / 3.0, y[1] / 3.0 + y[2] * 2.0 / 3, y[2] end
  for i = 1, n - 2 do
...
function line_counter()
  local line_no = 0
  local function same_line() return line_no end
...
function curve(model)
  ...
  if hascubicstore then
    d:set("cubic", cubicstore)
  else
    d:set("cubic", true)
  end
...
function make_axes(model, num)
  ...
  if xtickstore and (xtickstore ~= "") then
    ...
    if _G.type(xticklist[1]) == "table" then xticklist = xticklist[1] end
    for i, x in pairs(xticklist) do
      if isfinite(x) then
        if (x > x0) and (x < x1) then xticks[#xticks + 1] = x end
      end
    end
  ...
  if ytickstore and (ytickstore ~= "") then
    ...
    if _G.type(yticklist[1]) == "table" then yticklist = yticklist[1] end
    for i, y in pairs(yticklist) do
      if isfinite(y) then
        if (y > y0) and (y < y1) then yticks[#yticks + 1] = y end
      end
    end
  else -- place ticks at every integer
