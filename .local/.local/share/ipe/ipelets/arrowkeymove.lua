...
function mov(model, num)
  ...
  if pin.fixed or pin.horizontal and dx ~= 0 or pin.vertical and dy ~= 0 then
    model:warning("Cannot move objects", "Some object is pinned and cannot be moved")
    return
  end
  ...
  if last_undo and last_undo._is_movement and last_undo.pno == model.pno and last_undo.vno == model.vno then
...
function test(model, num) model.ui:explain "test run!" end
