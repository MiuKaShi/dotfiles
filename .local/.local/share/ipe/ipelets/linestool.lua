...
function _G.LINESTOOL:explain()
  local s
  if self:last() == SPLINE then
    s = ("Left: add ctrl point | Shift+Left: switch to line mode" .. " | Right: delete ctrl point")
  else
    s = "Left: add vtx | Right: delete vtx"
  end
  s = s .. " | Space, Ctrl-Left or Double-Left: final vtx"
  if self:has_segs(2) then s = s .. " | " .. _G.shortcuts_linestool.spline .. ": spline mode" end
  if self:has_segs(3) then s = s .. " | " .. _G.shortcuts_linestool.arc .. ": circle arc" end
...
function _G.LINESTOOL:delete()
  if #self.v > 2 then
    table.remove(self.v)
    table.remove(self.t)
    if self:last() == ARC then self.t[#self.t] = VERTEX end
...
function _G.LINESTOOL:key(text, modifiers)
  ...
  elseif text == " " or text == "Enter" then -- enter doesnt work :(
    if self:last() == SPLINE then
      self:finish(false)
    elseif self:last() == VERTEX and #self.t > 2 and self.t[#self.t - 1] == ARC then
      self:finish(false)
    else
      self:finish(true)
    end
  ...
  elseif self:has_segs(3) and text == _G.shortcuts_linestool.arc then
    ...
    self:explain()
    return true
  elseif #self.v > 2 and self.t[#self.t - 1] == VERTEX and text == _G.shortcuts_linestool.set_axis then
    -- set axis
...
function _G.LINESTOOL:finish(deleteLast)
  ...
  if not self.shape.closed then obj:set("pathmode", "stroked") end
