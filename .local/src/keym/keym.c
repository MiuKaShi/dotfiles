...
static const char* unmap[] = {"h", "j", "k", "l", "u", "d", "c", "x", "z", "w", "a", "s", "i","n", "m", "r", "f", "q", "Escape", "Shift_L", "semicolon", "backslash", "Tab", "Left", "Right", "Up", "Down", "Control_R"};
...
int main()
{
    ...
    while (1)
    {
        ...
        speed = 2;
        speed = (pressed(XK_c)) ? 4 : speed;
        speed = (pressed(XK_x) || pressed(XK_backslash) || pressed(XK_Tab)) ? 3 : speed;
        speed = (pressed(XK_semicolon) || pressed(XK_Shift_L)) ? 1 : speed;
        speed = (pressed(XK_z)) ? 0 : speed;

        /* mouse clicks */
        XTestFakeButtonEvent(display, Button1, (pressed(XK_n) || pressed(XK_a)) ? True : False, CurrentTime);
        XTestFakeButtonEvent(display, Button3, (pressed(XK_m) || pressed(XK_s)) ? True : False, CurrentTime);
        XTestFakeButtonEvent(display, Button2, (pressed(XK_i) || pressed(XK_w)) ? True : False, CurrentTime);
        XTestFakeButtonEvent(display, 8, pressed(XK_r) ? True : False, CurrentTime);
        XTestFakeButtonEvent(display, 9, pressed(XK_f) ? True : False, CurrentTime);

        /* exit */
        if (!pressed(XK_q) && !pressed(XK_Escape))
            quit = 1;

        if (++zzz > idle_cutoff || (quit == 1 && (pressed(XK_q) || pressed(XK_Escape))))
        {
