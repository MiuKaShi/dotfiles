...
class LPic:
    ...
    def preprocess_resize(self, img):
        ...
        if max_size:
            ...
            logger.debug(
                "max_w: {}, max_h: {}, new_w: {}, new_h: {}".format(
    ...
    def parse_url_prefix(self):
        ...
        if (
            urlparse(url_prefix).scheme in ("http", "https")
            and url_prefix.count("/") > 2
        ):
    ...
    def get_default_pic():
        ...
        files = list(
            filter(
                lambda f: os.path.splitext(os.path.abspath(f))[1].lower() in pic_suffix,
                files,
            )
        )
        if files:
    ...
    def upload_process(self, pic):
        ...
        if ret:
            ...
            logger.info(
                "已上传至{}：{}  {}K".format(
                    self.cloud_name, os.path.basename(file), size
                )
            )
    ...
    def handle_del(self, dest):
        ...
        if keys:
            ...
            logger.error(
                "{}的存储库里没有以'{}'开头的文件".format(self.cloud_name, prefix)
            )
