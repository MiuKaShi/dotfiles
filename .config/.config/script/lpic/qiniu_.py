...
class QiniuLPic(LPic):
    ...
    @property
    def web_url(self):
        return "https://portal.qiniu.com/bucket/{}/resource".format(
            self.cloud["Bucket"]
        )

    def upload(self, file, prefix=""):
        _token = self.client.upload_token(
            self.cloud["Bucket"], os.path.basename(file), 600
        )
    ...
    def list(self, prefix):
        ...
        ret, _, _ = bucket.list(
            self.cloud["Bucket"], prefix=prefix, limit=self.MAX_KEYS
        )
        return [
            i["key"]
            for i in sorted([i for i in ret["items"]], key=lambda x: -x["putTime"])
        ]
