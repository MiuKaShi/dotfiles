...
def short_title(s):
    s = s.split()[0][0].lower()+s.split()[-1][0].lower()
    return "".join(s)
...
def check_format_is_valid(format):
    ...
    if not tags:
        logger.error(
            f"The specified format does not contain any tag. Tags must be delimited by {{ and }}."
        )
        return None
    for tag in tags:
        if not tag in AllowedTags:
            logger.error(
                f'The specified format contains "{tag}", which is not a valid tag.'
            )
...
def build_filename(infos, format=None, tags=None):
    ...
    rep_dict = dict.fromkeys(
        tags
    )  # Initialize a dictionary with keys equal to the elements of the list tags, and all the values set to None
    ...
    if "{YYYY}" in rep_dict.keys():
        rep_dict["{YYYY}"] = (
            short_year(infos["year"])
            if ("year" in infos and is_valid_integer(infos["year"], 4))
            else "00"
        )
    ...
    if any(
        item in rep_dict.keys() for item in ListAuthorTags
    ):  # Chec if any of the tag chosen by the user is one of the author tags defined in ListAuthorTags
        if author_info:
            ...
            if isinstance(author_info, list):
                lastnames = [
                    author["family"] for author in author_info if "family" in author
                ]
                firstnames = [
                    author["given"] for author in author_info if "given" in author
                ]
                ...
                firstnames = [
                    ...
                    for name in authors
                ]  # The check on len(name.split())>1 is necessary to address the case in which                                                                                     #  the string name contains only one words (e.g. only the last name of the author is available)
            ...
            if lastnames:
                ...
                rep_dict["{Aetal}"] = lastnames[0].lower()

                if firstnames:
                    firstinitials = [
                        firstname[0][0].upper() + "." if len(firstname[0]) > 0 else ""
                        for firstname in firstnames
                    ]
                    firstinitial_lastnames = [
                        firstinitials + " " + lastname
                        for (firstinitials, lastname) in zip(firstinitials, lastnames)
                    ]
    ...
    if (
        config.get("max_words_title") > 0 and "title" in infos
    ):  # check if we need to limit the number of words in the title
        separator = " "
    ...
    if "{T}" in rep_dict.keys():
        if ("title" in infos) and infos["title"]:
            rep_dict['{T}'] = short_title(infos['title'])
        else:
            rep_dict["{T}"] = "[NoTitle]"

    # if '{Ts}' in rep_dict.keys():
    #    if ('title' in infos) and infos['title']:
    #        rep_dict['{Ts}'] = to_short(infos['title'])
    #    else:
    #        rep_dict['{Ts}'] = '[NoTitle]'
    #
