...
function make_segment(model, p1, p2)
  local shape = { type = "curve", closed = false, { type = "segment", p1, p2 } }
...
function ellipse_to_ellipse_tangent_segments(model, e1, e2)
  ...
  if #ints <= 1 then -- ellipses are not intersecting
    ...
    for i = 1, #ints do
      ...
      if
        is_point_in_ellipse(0.5 * (cx1[1] + cx2[1]), e1) == false
        and is_point_in_ellipse(0.5 * (cx1[1] + cx2[1]), e2) == false
      then
        p1[i], p2[i] = cx1[1], cx2[1]
      elseif
        is_point_in_ellipse(0.5 * (cx1[1] + cx2[2]), e1) == false
        and is_point_in_ellipse(0.5 * (cx1[1] + cx2[2]), e2) == false
      then
        p1[i], p2[i] = cx1[1], cx2[2]
      elseif
        is_point_in_ellipse(0.5 * (cx1[2] + cx2[1]), e1) == false
        and is_point_in_ellipse(0.5 * (cx1[2] + cx2[1]), e2) == false
      then
        p1[i], p2[i] = cx1[2], cx2[1]
      else
        p1[i], p2[i] = cx1[2], cx2[2]
      end
    end
  end
  ...
  if #ints == 0 then -- if ellipses are not intersecting
    ...
    for i = 1, 20 do
      for j = 1, #p1 do -- for all four possible tangent points
        ...
        if S(p1[j], pa):intersects(l) then
          p2[j] = pb
        else
          p2[j] = pa
        end
...
function print_selection_warning(model) model:warning "Must select at least two markers, circles, ellipses and/or arcs" end
