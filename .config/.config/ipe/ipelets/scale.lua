...
function scale_unscalable(model, obj, factor)
  if obj:type() == "reference" then scale_property(model, obj, "symbolsize", "symbolsize", factor) end
...
function scale_selection(model)
  ...
  if not str or str:match "^%s*$" or not str:match "^[%+%-%d%.]+$" then return end
  ...
  t.redo = function(t, doc)
    ...
    for i, obj, sel, layer in p:objects() do
      if sel then apply_recursively(p, i, function(obj) scale_unscalable(model, obj, factor) end) end
    end
  end
  model:register(t)
end
