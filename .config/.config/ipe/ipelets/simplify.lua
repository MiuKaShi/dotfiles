    ...
    for i, obj, sel, layer in p:objects() do
      if sel then
        ...
        for _, subPath in ipairs(shape) do
          if subPath["type"] == "curve" then simplifyPath(subPath, tolerance, num) end
        end
        obj:setShape(shape)
      end
    end
  ...
  t.redo = function(t, doc)
    ...
    for i, obj, sel, layer in p:objects() do
      if sel then
        local shape = obj:shape()
        for _, subPath in ipairs(shape) do
          if subPath["type"] == "curve" then convertPath(subPath) end
        end
        obj:setShape(shape)
      end
    end
...
function simplifyPath(path, tolerance, num)
  ...
  for i, point in ipairs(points) do
    if marked[i] then table.insert(new_points, point) end
  end
  ...
  t.redo = function(t, doc)
    ...
    for i, obj, sel, layer in p:objects() do
      if sel then
        local shape = obj:shape()
        for _, subPath in ipairs(shape) do
          if subPath["type"] == "curve" then roundPath(subPath) end
        end
        obj:setShape(shape)
      end
    end
