...
function draw_dash_bar(bar)
  ...
  for k = 1, 2 do
    for i = 1, coord[k] do
      if i == 1 then cairo_move_to(cr, start_x, bar.y) end
...
function update_cpu_graph() cpu_graph[update_num] = get_value "cpu" end
...
function get_percents(val, total) return val / total * 100 end

--- GPU % ---
function conky_gpu()
  local gpu = conky_parse "${exec nvidia-smi | grep % | cut -c 37-40}"
  if gpu == nil then gpu = 0 end
