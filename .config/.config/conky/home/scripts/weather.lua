...
local weather_data = { "", "", "" }
local need_count = 4
local xs = { 38, 188, 338, 488 } -- положение каждого элемента по горизонтали
...
function get_weather(key, type, params)
  local url = "curl -H 'X-Gismeteo-Token: "
    .. token
    .. "' 'https://api.gismeteo.net/v2/weather/"
    .. type
    .. "/"
    .. cityid
    .. "/"
...
function weather_now()
  if weather_data[1] == "" or os.date "%M:%S" == "00:01" then get_weather(1, "current", "?lang=en") end
  ...
  text_by_left({ x = 20, y = 930 }, temp, { color = "0x02c3fa", font = "LED", size = "88" })
  text_by_left({ x = 10, y = 970 }, comf, { font = "LED", size = "68" })
  text_by_right({ x = 383, y = 915 }, format_wind(response.wind.direction.scale_8), { font = "Arrows", size = 60 })
  text_by_right({ x = 423, y = 910 }, response.wind.speed.m_s, { font = "LED", size = "38" })
  text_by_right({ x = 483, y = 910 }, "m/s", { font = "LED", size = "22" })
  text_by_right({ x = 383, y = 950 }, response.pressure.mm_hg_atm, { font = "LED", size = "38" })
  text_by_right({ x = 483, y = 950 }, "мм.рт.ст.", { size = "22" })
  text_by_right({ x = 463, y = 975 }, response.description.full, { size = "20" })
  display_image { img = img_path .. response.icon .. ".png", coord = { x = 215, y = 890 } }
end
...
function weather_by_hours(now_date)
  if weather_data[2] == "" or os.date "%M:%S" == "00:06" then get_weather(2, "forecast", "?days=2") end
  local response = json.decode(weather_data[2]).response
  local counter = 1
  local ys = { 1015, 1035, 1080 } -- положение каждого жлемента по вертикали
  for i in pairs(response) do
    ...
    if
      ...
      and (
        ...
        or date.day > now_date.day
        or date.month > now_date.month
        or date.year > now_date.year
      )
    then
      ...
      display_image {
        img = min_img_path .. response[i]["icon"] .. ".png",
        coord = { x = xs[counter] - 35 / 2, y = ys[2] },
      }
...
function weather_by_days(now_date)
  if weather_data[3] == "" or os.date "%M:%S" == "00:11" then get_weather(3, "forecast/aggregate", "?days=5") end
  local response = json.decode(weather_data[3]).response
  local counter = 1
  local ys = { 1135, 1160, 1205, 1225 } -- положение каждого жлемента по вертикали
  for i in pairs(response) do
    local date = os.date("*t", response[i]["date"]["unix"])
    if
      counter <= need_count and (date.day > now_date.day or date.month > now_date.month or date.year > now_date.year)
    then
      ...
      text_by_center(
        { x = xs[counter] - 5, y = ys[3] },
        temp_max .. "°",
        { size = "20" },
        { color = temp_color(temp_max) }
      )
      text_by_center(
        { x = xs[counter] + 5, y = ys[4] },
        temp_min .. "°",
        { size = "20" },
        { color = temp_color(temp_min) }
      )
      display_image {
        img = min_img_path .. response[i]["icon"] .. ".png",
        coord = { x = xs[counter] - 35 / 2, y = ys[2] },
