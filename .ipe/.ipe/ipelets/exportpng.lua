...
function run(model)
  ...
  local s, f = ipeui.fileDialog(model.ui:win(), "save", "Export to PNG", filter_png, dir)
  if s then
    if config.toolkit ~= "cocoa" and ipe.fileExists(s) then
      local b = ipeui.messageBox(
        model.ui:win(),
        "question",
        "File already exists!",
    ...
    model.ui:renderPage(model.doc, model.pno, model.vno, "png", s, resolution / 72.0, true, false) -- transparent, nocrop
  end
end
----------------------------------------------------------------------
